...
export function Navbar({ lang, dict }: NavbarProps) {
  ...
  return (
    <nav className="fixed top-4 md:top-6 left-1/2 -translate-x-1/2 z-50 w-[min(calc(100%-1.5rem),1200px)]" aria-label="Main Navigation">
      <div className="glass flex items-center justify-between h-14 md:h-16 px-3 md:px-6 rounded-xl md:rounded-2xl relative z-50">
        {/* Logo */}
        <Link href={`/${lang}`} className="flex items-center gap-1.5 md:gap-2 group shrink-0 relative z-50" aria-label="Elite Hospital Home">
        ...
        <div className="flex items-center gap-2 md:gap-4 shrink-0 relative z-50">
          ...
          <div className="hidden sm:flex items-center gap-1 bg-muted/30 p-1 rounded-lg md:rounded-xl border border-border/40" role="group" aria-label="Language Selector">
            {i18n.locales.map((locale) => (
              <Link
                key={locale}
                href={redirectedPathname(locale)}
                aria-label={`Switch to ${locale === 'en' ? 'English' : 'Kannada'}`}
                className={cn(
          ...
          <Magnetic strength={0.3}>
            <button 
              className="hidden lg:flex h-10 px-6 rounded-xl bg-royal-cobalt text-white text-xs font-bold uppercase tracking-widest shadow-medical hover:scale-105 active:scale-95 transition-all whitespace-nowrap items-center gap-2"
          ...
          >
            <div className="flex flex-col gap-6">
              ...
              >
                ...
                <button 
                  className="h-16 w-full rounded-2xl bg-royal-cobalt text-white font-black uppercase tracking-[0.2em] text-sm shadow-xl shadow-royal-cobalt/20 active:scale-95 transition-all flex items-center justify-center gap-3"
                  aria-label={dict.navbar.book_appointment}
                >
                  {dict.navbar.book_appointment}
